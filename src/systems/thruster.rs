...
use amethyst::ecs::{ReadStorage, Join, Read, System, WriteStorage};
use nalgebra_glm::clamp_vec;

use specs_physics::{
    colliders::Shape,
    nphysics::{algebra::Velocity3, object::BodyStatus, math::{ForceType, Force}},
    nalgebra::*,
    parameters::Gravity,
...
impl<'a> System<'a> for ThrusterSystem {
    ...
    fn run(&mut self, (mut thrusters, mut bodies, transforms, time): Self::SystemData) {
        for (body, thruster, transform) in (&mut bodies, &mut thrusters, &transforms).join() {
            ...
            let linear_acceleration = transform.rotation() * Vector3::z().scale(thruster.thrust_force);
            // get angular acceleration
            let angular_acceleration = Vector3::y().scale(thruster.rotational_force);
            // apply forces
            body.apply_external_force(
                &Force::new(
            ...
            if thruster.thrust_control != 0f32 { linear_damping = Vector3::zeros() }
            if thruster.rotation_control != 0f32 { angular_damping = Vector3::zeros() }
            body.apply_external_force(
                &Force::new(
                    linear_damping,
                    angular_damping,
            ...
            if thruster.max_speed.is_some() {
                if body.velocity.linear.magnitude() > thruster.max_speed.unwrap() {
                    body.velocity.linear = body.velocity.linear.scale(thruster.max_speed.unwrap() / body.velocity.linear.magnitude());
                }
            }
            // Limit the force if a max speed is set
            if thruster.max_angular_speed.is_some() {
                if body.velocity.angular.magnitude() > thruster.max_angular_speed.unwrap() {
                    body.velocity.angular = body.velocity.angular.scale(thruster.max_angular_speed.unwrap() / body.velocity.angular.magnitude());
                }
            }
        }
    }
